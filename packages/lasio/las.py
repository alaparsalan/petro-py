...
class LASFile(object):
    ...
    * :meth:`lasio.LASFile.read` - control how NULL values and errors are
    ...
    def add_curve(self, mnemonic, data, unit="", descr="", value=""):
        ...
        try:
            # Check if the curve already exists by mnemonic
            curve_names = [curve.mnemonic for curve in self.curves]
            if mnemonic in curve_names:
                ix = curve_names.index(mnemonic)  # Find the index of the existing curve
                self.update_curve(ix=ix, data=data, unit=unit, descr=descr, value=value)
            else:
                # If the curve doesn't exist, append it
                self.append_curve(mnemonic, data, unit=unit, descr=descr, value=value)
        except AttributeError as e:
            raise AttributeError(f"Failed to add or update the curve '{mnemonic}'. {str(e)}")
